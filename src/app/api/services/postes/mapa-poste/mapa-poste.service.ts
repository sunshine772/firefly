...
export class MapaPostesService {
  ...
  modificar(poste: Postes) {
    if (this.map) {
      ...
      this.map.on('move', () => {
        ...
        poste.coordenadas.latitud = center.lat;
        poste.coordenadas.longitud = center.lng;
      });
    } else {
  ...
  mostrarPoste(poste: Postes): void {
    if (poste.coordenadas) {
      const { latitud, longitud } = poste.coordenadas;
  ...
  // adicionarposte(postees: any) {
  //   for (const poste of postees) {
  ...
  //     el.style.padding = '0';
  //     el.style.backgroundImage = `url('./assets/poste.png')`;
  ...
  //       draggable: false,
  //     })
  //       .setLngLat([poste.longitud, poste.latitud])
  //       .addTo(this.map);
  //   }
  // }
